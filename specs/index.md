...
- Guided Pi workflows use manifest `templates.paths` and record product-run status in `product-guidelines/spec-status.yaml`.
...
Template mappings live in `specs/manifest.yaml` under `templates.paths`. Guided spec creation prompts and skills must use those mappings instead of guessing filenames. Empty template lists mean the spec is generated by its manifest command or sub-agent workflow.
