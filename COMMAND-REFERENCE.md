...
Complete OpenAPI 3.0 specification with all endpoints, request/response schemas, authentication, error handling, API versioning & evolution guidance (breaking change matrix, Protobuf reserved fields, deprecation patterns, migration strategies). Every journey step has API support. Includes database-to-API type mapping (prevents precision loss), comprehensive validation rules, and security compliance (PII marking, GDPR export endpoints). Reads API design + journey + database schema + tech stack + architecture. Outputs: `08b-api-contracts.md` + `.ctx.md`
