# Brand Naming Discovery

> **Generated**: [Date]
> **Status**: [In Progress/Finalized]

## Overview

This document captures the brand naming discovery process, including candidates, trademark research, and finalist recommendations.

---

## Naming Strategy

### Direction

**Naming approach**: [Descriptive / Abstract / Metaphorical / Compound / Acronym]

**Reasoning**: [Why this approach fits the brand strategy and product]

### Naming Criteria

**Must have**:
- [ ] [Criterion 1 - e.g., "Easy to pronounce"]
- [ ] [Criterion 2 - e.g., "Memorable"]
- [ ] [Criterion 3 - e.g., ".com domain available"]

**Nice to have**:
- [ ] [Criterion 4 - e.g., "Works internationally"]
- [ ] [Criterion 5 - e.g., "Short (< 10 characters)"]

### Keywords and Themes

**Core concepts to explore**:
- [Keyword 1] - [Why relevant]
- [Keyword 2] - [Why relevant]
- [Keyword 3] - [Why relevant]

**Themes**:
- [Theme 1 - e.g., "Speed/velocity"]
- [Theme 2 - e.g., "Simplicity/clarity"]
- [Theme 3 - e.g., "Power/strength"]

### Names to Avoid

**Styles to avoid**:
- [Style 1 - e.g., "Generic tech names like 'TechSolutions'"]
- [Style 2 - e.g., "Made-up words with missing vowels like 'Naym'"]

**Competitor names** (for reference):
- [Competitor 1 name]
- [Competitor 2 name]
- [Competitor 3 name]

---

## All Candidates (20-30 names)

### Descriptive Names
*Names that describe what you do*

1. **[Name]** - [Brief rationale]
2. **[Name]** - [Brief rationale]
3. **[Name]** - [Brief rationale]
4. **[Name]** - [Brief rationale]
5. **[Name]** - [Brief rationale]

### Metaphorical Names
*Names that suggest the benefit*

1. **[Name]** - [Brief rationale]
2. **[Name]** - [Brief rationale]
3. **[Name]** - [Brief rationale]
4. **[Name]** - [Brief rationale]
5. **[Name]** - [Brief rationale]

### Abstract Names
*Invented or borrowed words*

1. **[Name]** - [Brief rationale]
2. **[Name]** - [Brief rationale]
3. **[Name]** - [Brief rationale]
4. **[Name]** - [Brief rationale]
5. **[Name]** - [Brief rationale]

### Compound Names
*Two words combined*

1. **[Name]** - [Brief rationale]
2. **[Name]** - [Brief rationale]
3. **[Name]** - [Brief rationale]
4. **[Name]** - [Brief rationale]
5. **[Name]** - [Brief rationale]

### Other Categories
*Acronyms, mashups, etc.*

1. **[Name]** - [Brief rationale]
2. **[Name]** - [Brief rationale]
3. **[Name]** - [Brief rationale]
4. **[Name]** - [Brief rationale]
5. **[Name]** - [Brief rationale]

---

## Trademark Research Summary

### Research Approach

**Sources checked**:
- [ ] USPTO database (trademarks.uspto.gov)
- [ ] Google search (existing usage)
- [ ] Domain availability (key TLDs)
- [ ] Social media handles

### Risk Categories

**🟢 Low Risk**: No similar trademarks, domain available, clear to use
**🟡 Medium Risk**: Some similar names but different industry, investigation needed
**🔴 High Risk**: Existing trademark in same space, domain taken, likely conflict

---

## 10 Finalists

### 1. [Name]

**Category**: [Descriptive/Metaphorical/Abstract/Compound]

**Meaning**: [What it means or suggests]

**Trademark Status**: 🟢/🟡/🔴 [Brief assessment]

**Domain Availability**:
- `.com`: ✅/❌ [Available/Taken]
- `.io`: ✅/❌
- `.ai`: ✅/❌
- Alternative: [If .com taken, what's available?]

**Pros**:
- [Pro 1]
- [Pro 2]
- [Pro 3]

**Cons**:
- [Con 1]
- [Con 2]

**Fits Brand Strategy**: [How it aligns with brand purpose, values, personality]

---

### 2. [Name]

**Category**: [Descriptive/Metaphorical/Abstract/Compound]

**Meaning**: [What it means or suggests]

**Trademark Status**: 🟢/🟡/🔴 [Brief assessment]

**Domain Availability**:
- `.com`: ✅/❌
- `.io`: ✅/❌
- `.ai`: ✅/❌
- Alternative: [If .com taken]

**Pros**:
- [Pro 1]
- [Pro 2]
- [Pro 3]

**Cons**:
- [Con 1]
- [Con 2]

**Fits Brand Strategy**: [How it aligns]

---

*[Repeat for finalists 3-10]*

---

## Top 3 Recommendations

### 🥇 First Choice: [Name]

**Why this is the top recommendation**:

[2-3 paragraphs explaining why this name is the best fit for the brand strategy, has good availability, and will serve the product long-term]

**Next steps if chosen**:
1. [Action 1 - e.g., "Secure .com domain immediately"]
2. [Action 2 - e.g., "Conduct formal trademark search"]
3. [Action 3 - e.g., "Reserve social handles"]

---

### 🥈 Second Choice: [Name]

**Why this is a strong alternative**:

[1-2 paragraphs explaining why this is the second choice]

**Considerations**: [Any concerns or trade-offs]

---

### 🥉 Third Choice: [Name]

**Why this is worth considering**:

[1-2 paragraphs explaining why this made top 3]

**Considerations**: [Any concerns or trade-offs]

---

## Decision Framework

### Evaluation Criteria

| Criterion | Weight | Name 1 | Name 2 | Name 3 |
|-----------|--------|--------|--------|--------|
| Memorable | High | [Score] | [Score] | [Score] |
| Pronounceable | High | [Score] | [Score] | [Score] |
| Unique | High | [Score] | [Score] | [Score] |
| Domain available | High | [Score] | [Score] | [Score] |
| Trademark clear | High | [Score] | [Score] | [Score] |
| Fits brand | High | [Score] | [Score] | [Score] |
| Short | Medium | [Score] | [Score] | [Score] |
| International | Medium | [Score] | [Score] | [Score] |
| **Total** | | [Total] | [Total] | [Total] |

### Testing the Name

**Questions to ask**:

1. **Say it out loud** - Is it easy to pronounce?
2. **Spell it** - Can someone spell it after hearing it once?
3. **Remember it** - Can you recall it 24 hours later?
4. **Google it** - What comes up? Any negative associations?
5. **International** - Does it mean something unfortunate in other languages?
6. **Trademark** - Is it clear in your industry?
7. **Domain** - Is a reasonable domain available?
8. **Social** - Are handles available?
9. **Gut check** - Does it feel right?

---

## Next Steps

- [ ] Review finalists with team/stakeholders
- [ ] Test pronunciation with target audience
- [ ] Conduct formal trademark search for top choice(s)
- [ ] Secure domain and social handles
- [ ] Run `/define-messaging` with chosen name
- [ ] Begin visual identity design

---

## Document Control

**Status**: [In Progress/Finalized]
**Last Updated**: [Date]
**Chosen Name**: [TBD / Name]
**Owner**: [Name/Role]
