...
### Step 8: Check Review Iteration Count

Before posting update comment, check how many review rounds have occurred:

**1. Count existing /review-pr comments**
```bash
# Count only comments generated by /review-pr command (start with "# PR Review:")
gh pr view $1 --json comments --jq '[.comments[] | select(.body | startswith("# PR Review:"))] | length'
```

**2. Calculate current review round**
```bash
# Current round = existing review count + 1 (this iteration)
current_round=$((review_count + 1))
```

**3. Check iteration limit**
- If current_round >= 6: Include fatigue warning in update comment
- Suggest synchronous discussion instead of continued review iterations
- Still post the changes, but add warning section

**Why this method:**
- Precise: Only counts actual `/review-pr` command outputs (not all PR comments)
- Immune to discussion comments, CI messages, or other bot comments
- Consistent with `/review-pr` iteration tracking

### Step 9: Update PR with Comment

Post comment to PR confirming changes:

**Standard comment (rounds 1-5):**
```bash
...
**⚠️ Review Fatigue Warning**

This PR has completed ${current_round} review iterations. Continued async review cycles may not be productive.

Consider:
1. Synchronous discussion about remaining issues
2. Pairing session to resolve complex feedback
3. Approving current state and opening follow-up issues

Ready for final review if this addresses all critical concerns."
```
...
- ✅ Tests passing ({X} tests)
- ✅ Linting passed
- ✅ Changes committed and pushed (commit: {commit-sha})
- ✅ Comment posted to PR

Note on Review Fatigue:
This was review round {current_round} (exceeding the typical 5-round limit).
The PR comment includes a fatigue warning suggesting synchronous discussion
or approval if all critical issues are resolved.

Consider:
- Approving the PR if critical concerns are addressed
- Scheduling a pairing session for remaining complex issues
- Opening follow-up issues for nice-to-have improvements
```
...
5. ✅ Review iteration count checked (using /review-pr comment count)
6. ✅ Changes committed and pushed
7. ✅ PR comment posted confirming changes (with fatigue warning if round 6+)
...
# 8. Counts /review-pr comments (finds 2 existing reviews → round 3)
# 9. Posts standard comment to PR (no warning, round 3 of 5)
# 10. Reports success to user
```

**Example with fatigue warning (round 6+):**
```bash
# User runs command
/address-review 789

# Claude executes:
# 1. Fetches PR #789 review comments
# 2. Categorizes: 2 HIGH, 1 MEDIUM
# 3. Checks out PR branch
# 4. Applies 2 HIGH priority changes
# 5. Applies 1 MEDIUM priority change
# 6. Runs tests + linting (all pass)
# 7. Commits changes to PR branch
# 8. Counts /review-pr comments (finds 5 existing reviews → round 6)
# 9. Posts comment with fatigue warning
# 10. Reports success with note about exceeding 5-round limit
```
