...
gh pr view [PR-number] --json comments --jq '[.comments[] | select(.body | startswith("# PR Review:"))] | length'
```

**Why this method:**
- Precise: Only counts actual `/review-pr` command outputs
- Immune to other bot comments, discussions, or CI messages
- Reliable: Consistent header format ensures accurate tracking

**2. Calculate Current Review Round**
```bash
# Current round = existing review count + 1 (this review)
current_round=$((review_count + 1))
```

**3. Check Iteration Limit**
- If current_round >= 6: Warn user about review fatigue
- Suggest human discussion instead of 6th automated review
- Include "Review Round X of 5" in comment header (e.g., "Round 6 of 5" triggers warning)

**4. Warning Message (if current_round >= 6)**
```
This PR has already received 5 automated reviews. This is the 6th review round - further automated reviews may not be productive.
...
7. **Check Iteration Count**
   - Count existing `/review-pr` comments (those starting with "# PR Review:")
   - Calculate current review round (count + 1)
   - Warn if current round >= 6 (meaning 5+ reviews already exist)
...
[Count existing reviews: 1 previous /review-pr comment found]
[Calculate current round: 1 + 1 = 2]
[Post to GitHub with "Review Round: 2 of 5"]
