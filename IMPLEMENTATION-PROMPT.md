...
1. `/refine-journey` → Defines user journey through progressive interrogation
2. `/choose-tech-stack` → Selects technology based on journey requirements
3. `/generate-strategy` → Creates mission, metrics, monetization, architecture
4. `/create-design` → Builds design system
5. `/generate-backlog` → Creates prioritized feature backlog
6. `/create-gh-issues` → Pushes issues to GitHub
...
**Key findings:**
- ✓ "Exceptionally well-architected framework"
- ✓ "Philosophy-first architecture is a masterpiece"
- ✓ "AI-optimized prompting is world-class"
- ✗ "Stops at backlog - no code scaffolding"
- ✗ "Missing go-to-market strategy"
- ✗ "No financial modeling or unit economics"
...
You are implementing issues from `REVIEW-BACKLOG.md` to complete the framework. This backlog represents the "missing 30%" that takes Stack-Driven from **70% → 95% complete**.
...
├── 04-monetization.md
├── 05-architecture.md
├── 06-[new-session].md         # Your new commands go here
├── 07-[new-session].md
└── ...
```
...
4. **Success Criteria:** What defines success?
   ...
   - Extract value ratio (should be ≥10:1)
...
1. **Does the journey involve time-series data (metrics, logs, events)?**
   - YES → Consider: TimescaleDB, InfluxDB
   - NO → Continue to 2

2. **Does the journey require complex relationships between entities?**
   - YES → Consider: PostgreSQL, MySQL
   - NO → Continue to 3

3. **Does the journey require flexible schema (rapid iteration)?**
   - YES → Consider: MongoDB, DynamoDB
   - NO → Default: PostgreSQL (safest choice)

4. **What is the expected scale (from tech stack analysis)?**
   - <100K rows → Any relational DB works
   - 100K-10M rows → PostgreSQL with read replicas
   - >10M rows → Consider sharding or specialized DB
...
1. Rename output file numbers (e.g., `06-` → `04-`)
...
- Channel 1: [Name] → [Cost per acquisition estimate]
- Channel 2: [Name] → [Cost per acquisition estimate]
...
**Validation:**
- [ ] LTV:CAC ratio ≥ 3:1 (minimum for venture scale)
...
| LTV | $2,400 | $80/mo × 30mo lifetime × 80% margin | Churn assumption untested |
| LTV:CAC | 4.8:1 | Above | Healthy if assumptions hold |
| Payback | 6 months | CAC / (MRR × margin) | Acceptable for B2B |
\`\`\`
```
...
4. **Iterative:** Failed validation → revise journey, try again
...
### Calculation:
- Pain point resonance: [X/10 users said yes] → [Score 0-3]
- Willingness to pay: [Y/10 said yes] → [Score 0-3]
- Aha moment clarity: [Z/10 understood value] → [Score 0-4]
...
**Next steps:** [Re-run journey if score <7, proceed if ≥7]
\`\`\`
...
**If score ≥ 7/10:**
✅ Proceed to Session 2 (`/choose-tech-stack`)

**If score 4-6/10:**
⚠️ Revise journey based on insights
⚠️ Re-interview 5 users with updated journey
⚠️ Pass if revised score ≥ 7

**If score < 4/10:**
❌ Pivot required - fundamental problem/solution mismatch
❌ Return to ideation
❌ Do not proceed without major journey revision
\`\`\`
```
...
❌ **Wrong:** "Use Kubernetes for deployment"
✅ **Right:** "IF journey requires >1M requests/day AND team has DevOps expertise, THEN consider Kubernetes. ELSE use Vercel/Render."

### 2. Assuming Context
❌ **Wrong:** "Set up authentication" (how? which method?)
✅ **Right:** "Read 05-architecture.md auth strategy. IF 'JWT', generate middleware. IF 'OAuth', configure provider."

### 3. Missing Traceability
❌ **Wrong:** "Use blue as primary color"
✅ **Right:** "From 06-brand-foundation.md: Personality is 'trustworthy' → Use blue (trust association)"

### 4. Incomplete Decision Trees
❌ **Wrong:** "Choose between SQL and NoSQL"
✅ **Right:** [Full decision tree with 4-5 criteria and examples]

### 5. Generic Examples
❌ **Wrong:** "Example: Create a User model"
✅ **Right:** "Example (from compliance-saas): ComplianceOfficer model with role-based access..."

### 6. Weak "What We DIDN'T Choose"
❌ **Wrong:** "We didn't choose MongoDB because we chose PostgreSQL"
✅ **Right:** "MongoDB would work for rapid schema iteration, but compliance documents have stable structure (regulations change slowly). Reconsider if document types become highly variable."
...
**Read:**
- Architecture (05-architecture.md) → monorepo vs multi-repo decision
- Backlog (08-backlog/) → how many distinct services/apps?

**Decision Tree:**

1. **How many deployable services?**
   - 1 service (web app) → Single repo
   - 2-4 services (web + API + worker) → Monorepo
   - 5+ services → Multi-repo (or monorepo with orchestration)

2. **Is there shared code between services?**
   - YES → Monorepo (or multi-repo with packages)
   - NO → Multi-repo

3. **Team size?**
   - 1-3 people → Monorepo (easier to manage)
   - 4-10 people → Monorepo with module boundaries
   - 10+ people → Consider multi-repo
...
2. ✅ Read `REVIEW-BACKLOG.md`
3. ✅ Read 2-3 existing commands to match style
4. ✅ Identify which issue from backlog you're implementing
...
- ✅ Journey-first (serves user experience)
- ✅ Generative (derives from previous sessions)
- ✅ Critical (includes "What We DIDN'T Choose")
- ✅ AI-optimized (detailed enough for autonomous execution)
- ✅ Actionable (outputs are usable, not just documentation)
